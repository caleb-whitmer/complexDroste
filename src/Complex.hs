...
instance Show (Complex r i) where       -- Show override:
  show (Complex r i) = 
    if   r == 0                         -- If the real part is zero:
    then (show i) ++ "i"                -- Then only show the imaginary part. 
    else case (signum i) of             -- Otherwise when the imaginary part is:
                                        -- 
...
fromPoint (a, b) = Complex              -- 
                   (realToFrac a)       -- 
                   (realToFrac b)       -- Get a Complex number from a 2D point.

fromComplex :: (Complex w w) -> (Float, Float)
fromComplex (Complex a b) = (a, b)      -- Get a 2D point from a Complex number.